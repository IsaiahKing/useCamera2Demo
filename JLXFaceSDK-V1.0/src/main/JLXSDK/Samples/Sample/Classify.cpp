...
#define  JLX_CLASSIFY_LOG    "JLX_classify_JNI"

#define  JLX_CLASSIFY_ALOG(...)  __android_log_print(ANDROID_LOG_INFO,JLX_CLASSIFY_LOG,__VA_ARGS__)
#define  JLX_CLASSIFY_ELOG(...)  __android_log_print(ANDROID_LOG_ERROR,JLX_CLASSIFY_LOG,__VA_ARGS__)
...
// Return: none.
//=======================================================================
Classify::Classify(const char *CoefDataFcFileName,const char *LabelFileName)
{
    // Get CPU number
#ifdef WIN32
    SYSTEM_INFO sysinfo;
    ...
    if(m_CPU_num <= 4){
        m_CPU_num = m_CPU_num - 2;
    }else{
        m_CPU_num = m_CPU_num - 1;
    }
#endif
...
int Classify::GtiClassify(float *InputData, int Count)
{
    ...
    memcpy(m_Fc6input,InputData,m_Fc6_input_size*sizeof(float));

    //LOG(ERROR) << "m_Fc6input--->is end";
    //unsigned long sTimeStart = GetTickCount();
    fcOut = gtiAccelerateFC(&fcOutLen);//FC 加速
    if(fcOut == nullptr)
        return -999;
    //LOG(INFO) << "gtiAccelerateFC spend time " << (float)(GetTickCount() - sTimeStart);//add 2
    ...
    for (int i = 0; i < num; ++i)
    {
        ...
        //index 只能比 m_Labels.size() 小,否则越界访问了
        lable_id = jlx::to_string(index) + " : " + m_Labels.at(index);
...
#if DEBUG_INFO
    LOG(ERROR) << "GtiClassify function--->is end";
#endif
    return maxN[0];
}
...
// Return: None.
//====================================================================
void Classify::gtiLoadLabelFile(const char *FileName)
{
    string line;
...
#ifdef ANDROID
        LOG(ERROR) << "GTI_JNI: ERROR: Label filename is null.";
#endif
        return;
    }
...
int Classify::gtiPreloadCoefData(const char *CoefDataFcFileName){
    ...
    if (CoefDataFcFileName == NULL)
    {
        ...
        #ifdef ANDROID
            LOG(ERROR) << "GTI_JNI: ERROR: CoefDataFcFileName is null.\n";
        #endif
        return FALSE;
    }
    ...
    if (fpIn == NULL)
    {
        ...
        #ifdef ANDROID
        LOG(ERROR) << "GTI_JNI: : Failed in open conv filter file." << CoefDataFcFileName;
        #endif
        return FALSE;
    }

    fread_s(&m_Fc6_input_size, 4, 4, 1, fpIn);
    fread_s(&m_Fc6_output_size, 4, 4, 1, fpIn);
    fread_s(&m_Fc7_input_size, 4, 4, 1, fpIn);
    fread_s(&m_Fc7_output_size, 4, 4, 1, fpIn);
    fread_s(&m_Fc8_input_size, 4, 4, 1, fpIn);
    fread_s(&m_Fc8_output_size, 4, 4, 1, fpIn);


    fseek(fpIn,0,SEEK_SET);
    uint8_t *read_buffer = new uint8_t[24];
    fread(read_buffer,8,3,fpIn);
    std::vector<int> r;
    mCrypt.crypt_block(read_buffer,8*3,0,r);
    delete []read_buffer;

    m_Fc6_input_size  = (DWORD)r.at(0);
    m_Fc6_output_size = (DWORD)r.at(1);
    m_Fc7_input_size  = (DWORD)r.at(2);
    m_Fc7_output_size = (DWORD)r.at(3);
    m_Fc8_input_size  = (DWORD)r.at(4);
    m_Fc8_output_size = (DWORD)r.at(5);

    if(m_Fc6_input_size > 25088 || m_Fc7_input_size < 0){
#if DEBUG_OUT_ERROR
        JLX_CLASSIFY_ELOG("GTI_JNI: check model file size error\n");
#endif
        exit(1);
    }
#if DEBUG_OUT_INFO
    JLX_CLASSIFY_ALOG("GTI_JNI: m_CPU_num = %d\n", m_CPU_num);
    JLX_CLASSIFY_ALOG("GTI_JNI: m_Fc6_input_size(%d), m_Fc6_output_size(%d)\n", m_Fc6_input_size, m_Fc6_output_size);
    JLX_CLASSIFY_ALOG("GTI_JNI: m_Fc7_input_size(%d), m_Fc7_output_size(%d)\n", m_Fc7_input_size, m_Fc7_output_size);
    JLX_CLASSIFY_ALOG("GTI_JNI: m_Fc8_input_size(%d), m_Fc8_output_size(%d)\n", m_Fc8_input_size, m_Fc8_output_size);
#endif
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc6coef!\n");
        #endif
        return FALSE;
    }
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc6bias!\n");
        #endif
        return FALSE;
    }
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc7coef!\n");
        #endif
        return FALSE;
    }
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc7bias!\n");
        #endif
        return FALSE;
    }
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc8coef!\n");
        #endif
        return FALSE;
    }
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Read m_Fc8coef (size = %d, m_Fc8_output_size*4 = %d) error!\n", (int)retval, (int)m_Fc8_input_size * m_Fc8_output_size * 4);
        #endif
    }
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc8bias!\n");
        #endif
        return FALSE;
    }
    //long curent1 = ftell(fpIn);
    //JLX_CLASSIFY_ALOG("Read pre file index : %ld",curent1);
    ...
    {
#ifdef ANDROID
        JLX_CLASSIFY_ELOG("Read Fc8bias (size = %d, m_Fc8_output_size*4 = %d) error!\n", (int)retval, (int)m_Fc8_output_size*4);
#endif
    }
#if DEBUG_OUT_INFO
    JLX_CLASSIFY_ALOG("Read Fc8bias (size = %d, m_Fc8_output_size*4 = %d) OK!\n", (int)retval, (int)m_Fc8_output_size*4);
#endif
    fclose(fpIn);
    ...
    {
#ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc6_out!\n");
#endif
        return FALSE;
    }
    ...
    {
#ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc7_out!\n");
#endif
        return FALSE;
    }
    m_Fc8_out = (float*)new BYTE[m_Fc8_output_size * sizeof(float)];
#if DEBUG_OUT_INFO
    JLX_CLASSIFY_ALOG("Fc8 Output size : %d",m_Fc8_output_size * sizeof(float));
#endif
    if (m_Fc8_out == NULL)
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for fc8_out!\n");
        #endif
        return FALSE;
    }
    ...
    {
        #ifdef ANDROID
        JLX_CLASSIFY_ELOG("Failed allocat memory for probs_out!\n");
        #endif
        return FALSE;
    }
    ...
    for (int i = 0; i < m_CPU_num; i++)
    {
        ...
        {
            #ifdef ANDROID
            JLX_CLASSIFY_ELOG("Failed allocat memory for layer6ptr [%d]!\n", i);
            #endif
            return FALSE;
        }
...
#if DEBUG_OUT_INFO
    JLX_CLASSIFY_ALOG("gtiPreloadCoefData  end");
#endif
    return TRUE;
}
...
{
    m_PredText = jlx::to_string(m_Predications[Index].second) + string(" - ") + m_Predications[Index].first;
