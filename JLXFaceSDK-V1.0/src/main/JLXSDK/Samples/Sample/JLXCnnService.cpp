...
int Samples::convolution(BYTE* img, const int imgLength) {
    static int Ret = 0;
    cnn_mutex_lock(&mOpenCvMutex);
    const int gtiResult = GtiHandleOneFrameFloat(m_Device, img, imgLength, m_ImageOutPtr, GtiGetOutputLength(m_Device));
    cnn_mutex_unlock(&mOpenCvMutex);
    if (gtiResult == JLX_GTI_CON_ERROR_CODE)
    {
        ...
        GtiOpenDevice(m_Device, (char*)(m_deviceName.c_str()));
        Ret = -95;
    }
#if DEBUG_OUT_INFO
...
int Samples::convolution(const cv::Mat& img) {
    static int bRet = 0;
    const int ConvIn_Channels = 3;
    const int ConvIn_ImgSize = 224;
    const int imageInLength = (ConvIn_ImgSize * ConvIn_ImgSize) * ConvIn_Channels;
    ...
    cnn_mutex_lock(&mOpenCvMutex);
    const int gtiResult = GtiHandleOneFrameFloat(m_Device, m_Img224x224x3Buffer, imageInLength, m_ImageOutPtr, GtiGetOutputLength(m_Device));
    cnn_mutex_unlock(&mOpenCvMutex);
