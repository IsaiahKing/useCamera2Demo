...
#ifdef ANDROID
    mthreadnum = sysconf(_SC_NPROCESSORS_ONLN);
    if(mthreadnum <= 4){
    	mthreadnum = 2;
    }else{
    	mthreadnum = 3;
    }
	LOG(INFO) << "detect model use threads : " << mthreadnum;
#endif
...
//		if(0 != access(param_files.at(i).c_str(),F_OK)){
//			LOG(ERROR) << "Not find file: " << param_files.at(i).c_str();
//		}
//	}
//	for(int i = 0; i< bin_files.size();i++){
//		if(0 != access(bin_files.at(i).c_str(),F_OK)){
//			LOG(ERROR) << "Not find file: " << bin_files.at(i).c_str();
//		}
//	}
...
void MTCNN::detectMaxWithAlign(ncnn::Mat& img_,std::vector<Bbox>& finalBbox_,std::vector<cv::Mat>& out_img_){
	...
	img_w = img.w;
	img_h = img.h;
    cv::Mat cv_img = cv::Mat::zeros(img_h,img_w,CV_8UC3);
    img_.to_pixels(cv_img.data,ncnn::Mat::PIXEL_RGB);
	...
	for (size_t i = 0; i < scales_.size(); i++)
	{
		...
		if (thirdBbox_.size() > 0) {
			...
			finalBbox_ = thirdBbox_;//if largest face size is similar,.

            //extract landmark
            std::vector<cv::Point> p1;
            for(int i = 0;i < 5;i++){
                cv::Point tmp;
                tmp.x = finalBbox_[0].ppoint[i];
                tmp.y = finalBbox_[0].ppoint[i+5];
                p1.push_back(tmp);
            }
            // std::cout<<p1.size();
            // std::cout<<src_point.size();
            // crop image
            cv::Rect roi;
            roi.x = max(finalBbox_[0].x1-22,0);
            roi.y = max(finalBbox_[0].y1-22,0);
            roi.width  = min(finalBbox_[0].x2-finalBbox_[0].x1+22,img_w);
            roi.height = min(finalBbox_[0].y2-finalBbox_[0].y1+22,img_h);
            // //similarity transform
            // cv::Mat img_crop = cv_img(roi);

            //add
			cv::Mat face_ = cv::Mat::zeros(224,224,CV_8UC1);
			//end

            cv::Mat R = cv::estimateRigidTransform(p1,src_point,false);
            if(R.rows == 2){
                cv::warpAffine(cv_img,face_,R,face_.size());
                //cv::warpPerspective(img_crop,out_img_,R,out_img_.size());
                out_img_.push_back(face_);
            }
			break;
		}
	}
